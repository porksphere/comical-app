 ...
 *
 * Trackers are registry-installed on-device exactly like bridges (no separate build/bundle step —
 * see `installedTrackerStore` in `apps/mobile/src/data/embedded/stores.ts`), so there's nothing to
 * pre-generate for them here.
 *
