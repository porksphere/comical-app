...
download ≈ **200 MB** (never in the base binary). Platform OCR (Vision / ML Kit) stays in the
...
| --- | --- | --- | --- |
| 1. Detection | **comic-text-detector** (dmMaze, ONNX) | ~80 MB | Purpose-built on ~13 k manga/comic pages; returns bboxes, text *lines*, and pixel masks (masks are the runway to future text-removal/typesetting). Script-agnostic. GPL-3.0 — accepted per product decision. |
...
recent phones (detector once per page at ~1024 px; manga-ocr per detected region), which
