/**
 * Whether a newer build exists than the one running, for the three public end-user channels only
 * (ios-release, android-release/android-main, web-pages). Every other channel — the internal/dev
 * lanes (ios-main, ios-pr, ios-devclient, android-pr, *-e2e, local-dev) — is `'unsupported'` and
 * triggers no network request at all (`enabled: false` below).
 *
 * Per-channel check:
 *  - ios-release: compare the public AltStore/SideStore source's `apps[0].version` (a plain X.Y.Z
 *    string, same format as APP_VERSION on this channel — see build-ios-reusable.yml's "Compute
 *    full version") against APP_VERSION via `compareVersions`.
...
const SUPPORTED_CHANNELS = new Set(['ios-release', 'android-release', 'android-main', 'web-pages']);
 ...
 *  compares is minted by CI as a plain `vX.Y.Z` tag, never a pre-release suffix, on the one channel
 *  (ios-release) that uses it. */
...
async function checkIosRelease(signal?: AbortSignal): Promise<AppUpdateCheck> {
  const res = await fetch(IOS_RELEASE_APPS_JSON_URL, { signal });
...
async function fetchAppUpdateCheck(signal?: AbortSignal): Promise<AppUpdateCheck> {
  if (BUILD_CHANNEL === 'ios-release') return checkIosRelease(signal);
