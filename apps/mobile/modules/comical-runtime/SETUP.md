...
`.build/{id}/dist/tracker.js`). **This generated file is committed to the repo, not fetched in
CI** — deliberately, so the build never depends on a live external repo. Whenever
`comical-trackers` changes, regenerate and commit it locally:
```sh
cd ../comical-trackers && bun run build
cd apps/mobile && bun run build:tracker-bundles
git add src/data/embedded/tracker-bundles.generated.json && git commit
```
`build-ios-reusable.yml` / `build-android-reusable.yml` just sanity-check the committed file isn't
empty before the native build. Like the harness, it's baked in at build time: a Metro reload alone
won't pick up a regenerated one, only a rebuilt/reinstalled app.
